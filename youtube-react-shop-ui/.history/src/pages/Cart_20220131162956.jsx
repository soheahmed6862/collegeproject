...
import { removeproduct, removeproducts } from "../redux/apiCalls";
...
const Cart = () => {
 ...
 try {
  const res=await userReuest.post("/chackout/p",
  {
...
navigation('/success',{state: res.data})
    ...
    <Container>
      ...
      <Wrapper>
        ...
        <Bottom>
          ...
          {
            carts.products.map((item,index) =>  <Product>
              <ProductDetail key={index}>
              <Image src={`http://localhost:5000/uploads/${item.image}`} />
                <Details>
                  <ProductName>
                  ...
                  {/* <button onClick={()=> productRemove(item._id)}>remove product </button> */}
                </Details>
              </ProductDetail>
              <PriceDetail>
